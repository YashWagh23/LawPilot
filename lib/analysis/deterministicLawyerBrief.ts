...
/**
 * LawPilot Disclaimer for Lawyer Briefs when no legal authority has been verified.
 * Never implies verified statutory context or verified legal authority.
 */
export const LAWYER_BRIEF_UNVERIFIED_DISCLAIMER =
  "NOTICE & DISCLAIMER: This briefing document was generated by LawPilot to assist the user in preparing for an efficient consultation with a licensed legal practitioner. It organizes facts and clause excerpts identified in the submitted document (no legal authority was verified; analysis is based on factual agreement text alone). This document does NOT constitute formal legal advice, representation, or an attorney-client relationship. All legal interpretations and strategic decisions must be confirmed by a licensed attorney.";
...
export function generateDeterministicLawyerBrief(
  ...
  const verificationState = getReportVerificationState({ evidenceChains });
  const isUnverified = verificationState.tone === "unverified" || verifiedLegalContext.length === 0;
  ...
  return {
    ...
    document: {
      ...
      jurisdiction: input.jurisdiction || (input.jurisdictionContext && input.jurisdictionContext.country !== "Unknown" ? formatJurisdictionBadge(input.jurisdictionContext) : "Not established by the document"),
    ...
    questionsForCounsel,
    importantDates,
    disclaimer: isUnverified ? LAWYER_BRIEF_UNVERIFIED_DISCLAIMER : LAWYER_BRIEF_STANDARD_DISCLAIMER,
  };
}
